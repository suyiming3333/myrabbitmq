...
@Component
public class OrderRecieverDirect2 {

    //配置监听队列 direct 将消息投递到 bindingkey与routing-key对应的队列（type 默认为direct）
    @RabbitListener(bindings = @QueueBinding(
            value = @Queue(value = "order-queue-direct2",durable = "true"),
            exchange = @Exchange(value = "order-exchange-direct2",durable = "true"))
            //key = "order.*"
    )
    ...
    public void onOrderMessage(@Payload Order order,//消息的内容
        ...
        System.out.println("---------order-queue-direct2收到消息-----------");
